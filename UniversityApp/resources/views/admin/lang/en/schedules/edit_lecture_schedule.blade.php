...
<div class="container">
 ...
 <div class="card">	
<div class="card-header">{{ __('Edit Lectures Schedule') }}</div>
   {!! Form::open(['action'=> ['Admin\AdminSchedulesController@updateLec', $lecsSchedules->id],'method' => 'PUT']) !!}
    <table>
    <tr>
    <td>&nbsp;Semester:&nbsp;</td>
    <td>
    <select name="semester" id="semester" class="form-control">
      ...
      <option value="1" @if($lecsSchedules->semester_no == 1) selected @endif>1</option>
      <option value="2" @if($lecsSchedules->semester_no == 2) selected @endif>2</option>
      </select>
    </td>
    <tr>
    </table>
 ...
 @for($i=0; $i<count($lecSchedule); $i++)
    ...
    <select name="course[]" id="course" class="form-control">
      ...
      <option value="{{$course->course_name}}" @if($course->course_name == $lecSchedule[$i]->course_name) selected @endif>{{$course->course_name}}</option>
      @endif
    ...
    <select name="seclec[]" id="seclec" class="form-control">
      ...
      <option value="Section" @if($lecSchedule[$i]->instructor_name != NULL) selected @endif>Section</option>
      <option value="Lecture" @if($lecSchedule[$i]->professor_name != NULL) selected @endif>Lecture</option>
      </select>
      </td>
      <td>
    @if($lecSchedule[$i]->instructor_name != NULL)
    {{Form::text('proinc[]',$lecSchedule[$i]->instructor_name,['class' => 'form-control', 'placeholder' => 'Professor/Instructor'])}}
     @else
     {{Form::text('proinc[]',$lecSchedule[$i]->professor_name,['class' => 'form-control', 'placeholder' => 'Professor/Instructor'])}}
     @endif
     </td>
     <td>
    {{Form::text('time_from[]',$lecSchedule[$i]->time_from,['class' => 'form-control', 'placeholder' => 'Time From'])}}
    </td>
    <td>
    {{Form::text('time_to[]',$lecSchedule[$i]->time_to,['class' => 'form-control', 'placeholder' => 'Time To'])}}
    </td>
    <td>
    <select name="day[]" id="day" class="form-control">
      <option value="">Select Day</option>
      <option value="Saturday" @if($lecSchedule[$i]->day == "Saturday") selected @endif>Saturday</option>
      <option value="Sunday" @if($lecSchedule[$i]->day == "Sunday") selected @endif>Sunday</option>
      <option value="Monday" @if($lecSchedule[$i]->day == "Monday") selected @endif>Monday</option>
      <option value="Tuesday" @if($lecSchedule[$i]->day == "Tuesday") selected @endif>Tuesday</option>
      <option value="Wednesday" @if($lecSchedule[$i]->day == "Wednesday") selected @endif>Wednesday</option>
      <option value="Thursday" @if($lecSchedule[$i]->day == "Thursday") selected @endif>Thursday</option>
      </select>
      </td>
      </tr>
      @endfor
