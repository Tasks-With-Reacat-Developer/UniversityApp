...
<div class="container">
 ...
 <div class="card">	
<div class="card-header">{{ __('Edit Exam Schedule') }}</div>
   {!! Form::open(['action'=> ['Admin\AdminSchedulesController@updateExam', $examsSchedules->id],'method' => 'PUT']) !!}
    <table>
    <tr>
    <td>&nbsp;Semester:&nbsp;</td>
    <td>
    <select name="semester" id="semester" class="form-control">
      ...
      <option value="1" @if($examsSchedules->semester_no == 1) selected @endif>1</option>
      <option value="2" @if($examsSchedules->semester_no == 2) selected @endif>2</option>
      </select>
    </td>
    <tr>
    </table>
 ...
 @for($i=0; $i<count($examSchedule); $i++)
    ...
    <select name="course[]" id="course" class="form-control">
      ...
      <option value="{{$course->course_name}}" @if($course->course_name == $examSchedule[$i]->course_name) selected @endif>{{$course->course_name}}</option>
      @endif
     ...
     <td>
    {{Form::text('time_from[]',$examSchedule[$i]->time_from,['class' => 'form-control', 'placeholder' => 'Time From'])}}
    </td>
    <td>
    {{Form::text('time_to[]',$examSchedule[$i]->time_to,['class' => 'form-control', 'placeholder' => 'Time To'])}}
    </td>
    <td>
    <select name="day[]" id="day" class="form-control">
      <option value="">Select Day</option>
      <option value="Saturday" @if($examSchedule[$i]->day == "Saturday") selected @endif>Saturday</option>
      <option value="Sunday" @if($examSchedule[$i]->day == "Sunday") selected @endif>Sunday</option>
      <option value="Monday" @if($examSchedule[$i]->day == "Monday") selected @endif>Monday</option>
      <option value="Tuesday" @if($examSchedule[$i]->day == "Tuesday") selected @endif>Tuesday</option>
      <option value="Wednesday" @if($examSchedule[$i]->day == "Wednesday") selected @endif>Wednesday</option>
      <option value="Thursday" @if($examSchedule[$i]->day == "Thursday") selected @endif>Thursday</option>
      </select>
      </td>
      </tr>
      @endfor
