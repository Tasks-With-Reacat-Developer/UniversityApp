...
<div class="card-header">{{ __('Edit Student Data') }}</div>
   ...
   <td><input type="text" class="form-control" value="{{$student->student_id}}" disabled></td>
        ...
        <tr>
   <td>&nbsp;GPA:&nbsp;</td>
   <td><input type="text" class="form-control" value="@if($student_data->gpa == NULL) 0.00 @else {{$student_data->gpa}} @endif" disabled></td>
    </tr>
   </table>
   {!! Form::open(['action'=> ['Admin\AdminStudentsController@update_student_data', $student_data->id, $student->id],'method' => 'PUT']) !!}
        <div class="selects">
        <table>
        <tr>
        <td>&nbsp;College:&nbsp;</td>
       ...
       <option value="{{$college->id}}" @if($college->id == $student_data->college) id="college-id" selected @endif>{{$college->college_name}}</option> 
       @endforeach 
       </select>
       </td>
       </tr>
     ...
     <select name="department_id" id="department" class="form-control">
      <option value="">Select Department</option>
       ...
       <option value="{{$department->id}}" @if($department->id == $student_data->department) id="department-id" selected @endif>{{$department->department_name}}</option> 
       @endif
       @endforeach
      </option>
      </select>
      ...
      <select name="level" id="level" class="form-control">
       ...
       <option value="{{$i}}" @if($student_data->level == $i) id="level-id" selected @endif>{{$i}}</option> 
        @endfor
       </option>
      </select>
      </td>
   ...
   <th>Grade</th>
    ...
    @for($c=0; $c<count($student_course);  $c++)
    <tr id = "course_elements{{$c}}">
    <td>
    <select name="course[]" id="course{{$c}}" class="form-control">
     <option value="">Select Course</option>
     <option value="{{$student_course[$c]->course_id}}" selected>{{$student_course[$c]->course->course_name}}</option>
     @foreach($courses as $course)
     @if($course->department_id == $students_data[0]->department)
     @if($course->id != $student_course[$c]->course_id)
    ...
    <td>
    {{Form::number('mid_term_grade[]',$student_course[$c]->mid_term_grade,['class' => 'form-control', 'placeholder' => 'Mid-Term Grade'])}}
    </td>
    <td>
     <select name="grade[]" id="grade{{$c}}" class="form-control">
      <option value="">Select Grade</option>
      <option value="4.0" @if($student_course[$c]->grade == 4.0) selected @endif>A</option>
      <option value="3.7" @if($student_course[$c]->grade == 3.7) selected @endif>A-</option>
      <option value="3.3" @if($student_course[$c]->grade == 3.3) selected @endif>B+</option>
      <option value="3.0" @if($student_course[$c]->grade == 3.0) selected @endif>B</option>
      <option value="2.7" @if($student_course[$c]->grade == 2.7) selected @endif>B-</option>
      <option value="2.3" @if($student_course[$c]->grade == 2.3) selected @endif>C+</option>
      <option value="2.0" @if($student_course[$c]->grade == 2.0) selected @endif>C</option>
      <option value="1.7" @if($student_course[$c]->grade == 1.7) selected @endif>C-</option>
      <option value="1.3" @if($student_course[$c]->grade == 1.3) selected @endif>D+</option>
      <option value="1.0" @if($student_course[$c]->grade == 1.0) selected @endif>D</option>
      <option value="0.7" @if($student_course[$c]->grade == 0.7) selected @endif>D-</option>
      <option value="0.0" @if($student_course[$c]->grade == 0.0) selected @endif>F</option>
      </option>
      </select>     
      </td>
      <td>
      @if($student_course[$c]->grade == NULL)
      <select name="course_status[]" id="cs_status" class="form-control">
      <option value="">Select Status</option>
      <option value="Active" @if($student_course[$c]->course_status == 'Active') selected @endif>Active</option>
      <option value="Deactived" @if($student_course[$c]->course_status == 'Deactived') selected @endif>Deactived</option>
      </select>
 {!! Form::hidden('course_status_pos[]', $c) !!}
      @endif
      </td>
     </tr>
    @endfor
...
var i = 1;
var c = 0;
var count = {{count($student_course)}};
...
$('#department').change(function(){
  var counter;
  var id = $(this).val();
  var std_value = {{count($student_course)}};
